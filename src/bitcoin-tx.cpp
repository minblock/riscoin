...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " riscoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  riscoin-tx [options] <hex-tx> [commands]  Update hex-encoded riscoin transaction\n" +
            "or:     riscoin-tx [options] -create [commands]   Create hex-encoded riscoin transaction\n" +
            "\n";
